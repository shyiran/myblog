...
func InsertAlbum(album Album) (int64, error) {
	return common.ModifyDB("insert into tb_album(filepath,filename,status,createtime)values(?,?,?,?)",
		album.Filepath, album.Filename, album.Status, album.Createtime)
}

//--------查询图片----------
func FindAllAlbums() ([]Album, error) {
	rows, err := common.QueryDB("select id,filepath,filename,status,createtime from tb_album")
	if err != nil {
		return nil, err
	}
	var albums []Album
